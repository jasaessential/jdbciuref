...
export default function XeroxPageClient() {
  ...
  const subtotal = useMemo(() => {
    ...
    const uploadSingleDocument = async (doc: DocumentState): Promise<string | null> => {
        if (doc.file.size > 50 * 1024 * 1024) { // 50MB
             setUploadStatus(prev => ({
                ...prev,
                [doc.id]: { status: 'skipped', progress: 100, url: null }
            }));
            return null;
        }
        ...
        try {
            ...
            console.error("One or more uploads failed.", error);
             toast({
                variant: "destructive",
                title: "Upload Failed",
                description: "One or more document uploads failed. Please review and retry, or proceed to checkout.",
                duration: 7000,
            });
            setIsUploading(false); // Stop loader to show errors
        }
    };
    ...
    useEffect(() => {
        ...
        const allFinished = Object.values(uploadStatus).every(s => s.status !== 'pending' && s.status !== 'uploading');
        const anyErrors = Object.values(uploadStatus).some(s => s.status === 'error');

        if (allFinished && !anyErrors) {
            storeJobsAndRedirect();
        }
    }, [uploadStatus, isUploading, storeJobsAndRedirect]);
        ...
        <CardContent className="space-y-4">
          ...
          {deliveryCharge > 0 && orderSettings && (
           ...
           <Button 
            ...
            >
                {isUploading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <CheckCircle className="mr-2 h-5 w-5" />}
                {isUploading ? 'Processing...' : 'Confirm & Proceed to Checkout'}
            </Button>
        </CardContent>
      </Card>
    );
